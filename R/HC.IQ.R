...
#' @export
HC.IQ <- function (y, covs = NULL, otu.tab, tree, tau = -1, score, hs = c(1, 3, 5, 7, 9), W = TRUE, comp = FALSE, CLR = FALSE, opt.ncl = 30, n.perm = 5000, seed = 123) {
  if (!comp) {
  ...
  if(tau == -1){
    ...
    if(is.null(covs)){
      null.fit <- rq(y ~ 1, tau = -1)
    } else{
      null.fit <- rq(y ~ ., tau = -1, data = as.data.frame(covs))
    }
    
    null.fit <- MiRKAT:::transfer_v(null.fit)
    u.obj <- MiRKAT:::NewRanks(null.fit, score = score)
    u <- u.obj$ranks / sqrt(u.obj$A2)
    ...
    if(is.null(covs)){
      ...
      fit <- rq(y ~ ., tau = tau, data = as.data.frame(covs))
    }
  ...
  n.Zs <- as.numeric(u %*% otu.tab)
  n.Z0s <- lapply(u.s, function(x) as.numeric(x %*% otu.tab))
  U0s <- lapply(apply(sapply(u.s, function(x) return(x %*% otu.tab)), 1, list), unlist)
  se <- sapply(U0s, sd)
  Zs <- n.Zs/se
  ...
  if (W) {
    ...
    return(list(graph.els = list(otu.ids = otu.ids, exp.HC = exp.HC, obs.HC = obs.HC, exp.wHC = exp.wHC, obs.wHC = obs.wHC), opt.nclust = k.best, ind.pvs = item.pvs, ada.pvs = ada.pvs))
  }
  if (!W) {
    ...
    exp.HC <- (Is/p)/sqrt(pvs * (1 - pvs)/p)
    obs.HC <- pvs/sqrt(pvs * (1 - pvs)/p)
    return(list(graph.els = list(otu.ids = otu.ids, exp.HC = exp.HC, obs.HC = obs.HC), ind.pvs = item.pvs, ada.pvs = ada.pvs))
  }
}
